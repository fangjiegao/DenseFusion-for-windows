...
parser.add_argument('--dataset_root', type=str, default='D:\Linemod_preprocessed',
...
parser.add_argument('--refine_margin', default=0.015, help='margin to start the training of iterative refinement')  # 0.013
...
def main():
    ...
    for epoch in range(opt.start_epoch, opt.nepoch):
        ...
        for j, data in enumerate(testdataloader, 0):
            ...
            test_dis += dis.item()
            logger.info('Test time {0} Test Frame No.{1} index.{2} dis:{3}'.format(
                time.strftime("%Hh %Mm %Ss", time.gmtime(time.time() - st_time)), test_count, idx, dis))
