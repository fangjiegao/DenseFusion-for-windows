...
def loss_calculation(pred_r, pred_t, target, model_points, idx, points, num_point_mesh, sym_list):
    ...
    if idx[0].item() in sym_list:
        ...
        # target = torch.index_select(target, 1, torch.tensor(inds).cuda().view(-1) - 1)
        ...
        # dis = torch.max(torch.norm((pred - target), dim=2), dim=1)
        # dis = torch.mean(distances_tensor, dim=1).squeeze()  # 无法求导
    else:
        dis = torch.mean(torch.norm((pred - target), dim=2), dim=1)
